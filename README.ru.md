...
заказы, возвраты, отзывы, финансы — 156 инструментов поверх Ozon Seller API и
Performance API.
...
порядка двадцати кабинетов, 156 инструментов. Обновляется он по мере собственной
...
токенами в каждом запросе. 156 инструментов Ozon весит **12 344 токена** против
...
**Профили инструментов.** Клиенту без tool search каталог из 156 инструментов
...
| `OZON_TOOLSETS` | пусто | профили инструментов через запятую: `pricing`, `ads`, `catalog`, `orders`, `analytics`, `feedback`, `finance`; пусто — все 156 |
...
└── ozon_mcp/
    ├── server.py        # MCP-сервер: 156 инструментов, мульти-магазин
...
## От доступа к API — к работающему репрайсеру

Этот сервер даёт модели доступ к кабинету. Решать, *какой* должна быть цена, —
отдельная задача, и её решает [**ozon-wildberries-repricer**](https://github.com/DeviceIngineering/ozon-wildberries-repricer):
держит эталонную цену, считает безубыточность по реальным тарифам каждой площадки,
выводит товары из акций, которые уводят их под себестоимость, и умеет искать
лучшую цену управляемым экспериментом на живых продажах.

Работает с Ozon, Wildberries и Яндекс.Маркетом в одном месте, и у него есть
собственный HTTP-контур для LLM-агентов — с рельсами, которых модели не хватает:
изменение цены, которое уронило бы товар в карантин Wildberries, проходит за
несколько прогонов; через три минуты цена перечитывается с площадки, потому что
площадки отвечают успехом на изменения, которых не сделали; несколько агентов на
одном каталоге не затирают решения друг друга.

Посмотреть можно без подключения магазина: `npm run demo` наливает синтетический
каталог и запускает приложение.
