...
Команды в терминале, которая добавила бы сервер, нет ни там, ни там: AI Assistant
настраивается только через Settings, Junie — файлом `.junie/mcp/mcp.json` и
слэш-командой `/mcp` внутри самого Junie.
