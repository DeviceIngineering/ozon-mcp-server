...
## Способ 1 — команда Cline CLI (только без токена)

Если установлен Cline CLI:

```bash
cline mcp install ozon --transport sse http://localhost:8000/sse
```

(`cline mcp add` — алиас той же команды.) Учтите две вещи: команда открывает
**интерактивный визард** и требует TTY, то есть в скрипт её не завернуть; флага
для произвольного заголовка у неё нет. С `MCP_AUTH_TOKEN` идите способом 2 или
укажите токен прямо в URL: `http://localhost:8000/sse?token=<MCP_AUTH_TOKEN>`.

Конфигурация Cline CLI живёт в `~/.cline/mcp.json` — это отдельный файл, не тот,
что у расширения VS Code.

## Способ 2 — файл конфигурации расширения
